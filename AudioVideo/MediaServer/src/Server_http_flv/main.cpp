...
const std::string file_flv = "../../resources/SampleVideo_1280x720_30mb.flv";

int main()
{
    int         port     = 8080;
    LOGI("httpflvServer http://127.0.0.1:%d/SampleVideo_1280x720_30mb.flv", port);
    ...
    */
    ...
    while (true)
    {
        ...
        LOGI("find new connect clientFd=%d", clientFd);
        ...
        FILE* fp;
        fp = fopen(file_flv.c_str(), "rb");
        if (!fp)
        {
            LOGE("fopen %s fail!", file_flv.c_str());
        }
        else
        {
            int times = 0;
