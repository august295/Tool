...
static void handle_accpet(int epollfd, int listenfd);                                                 // 处理接收到的连接
...
{
    int listenfd;
    listenfd = socket_bind("127.0.0.1", SER_PORT); // 创建套接字并进行绑定
...
static int socket_bind(const char* ip, int port)
{
    ...
    servaddr.sin_addr.s_addr = htonl(INADDR_ANY); // 指定 IP 地址 inet_addr("127.0.0.1")
